  ...
  return (
    <div>
      <button
        ...
        className={`flex w-full items-center gap-1.5 rounded px-2 py-1 text-left text-xs hover:bg-white/[0.06] ${
          isSelected ? "bg-emerald-400/10 text-emerald-300" : "text-slate-300"
        }`}
        title={node.path}
      >
        {isDir && (
        ...
        <div>
          ...
          {node.children.length === 0 && (
            <p
              ...
              className="py-1 text-[11px] text-slate-600"
            >
              (empty)
            </p>
          )}
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0a0c12]/90">
      <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.03] px-4 py-3">
        <h2 className="font-mono text-xs text-slate-500">explorer</h2>
        <button
          type="button"
          onClick={loadTree}
          className="rounded-md border border-white/10 px-2 py-0.5 text-[11px] text-slate-400 hover:bg-white/[0.06] hover:text-slate-200"
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {loading && <p className="px-2 py-1 text-xs text-slate-600">Loading files…</p>}
        {error && !loading && (
          <p className="px-2 py-1 text-xs text-red-400">{error}</p>
        )}
        {!loading && !error && root && root.children && root.children.length === 0 && (
          <p className="px-2 py-1 text-xs text-slate-600">This workspace is empty.</p>
        )}
